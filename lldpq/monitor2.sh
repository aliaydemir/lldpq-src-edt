#!/usr/bin/env bash
# Monitor Script - scoped analysis prototype
...
show_usage() {
    cat <<'EOF'
Usage: monitor2.sh [OPTIONS]

Collect switch telemetry and regenerate analysis reports. With no --only
option, monitor2.sh preserves the existing monitor.sh behavior and runs every
...
Examples:
  ./monitor2.sh
  ./monitor2.sh --only bgp
  ./monitor2.sh --only pfc-ecn
  ./monitor2.sh -s
