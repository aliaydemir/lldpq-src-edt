...
class OpticalCollectionIsolationTests(unittest.TestCase):
    ...
    def test_timeout_becomes_unknown_row_and_partial_host_coverage(self):
        ...
        self.assertIn('data-coverage-collected="1"', report)
        self.assertIn('data-coverage-failed-hosts="1"', report)
        self.assertIn('id="unknown-ports">1</div>', report)
        self.assertIn('id="unplugged-ports">0</div>', report)
        self.assertIn("filterPorts('UNKNOWN')", report)
