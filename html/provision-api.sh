...
def parse_ztp_password():
    try:
        with open(ZTP_SCRIPT_FILE, 'r') as f:
            content = f.read()
        m = re.search(r"echo\s+'cumulus:(.*?)'\s*\|\s*chpasswd", content)
        if m:
            return m.group(1)
    except Exception:
        pass
...
def write_upgrade_ztp_script():
    pub_key, _, _ = get_ssh_key_info()
    password = parse_ztp_password()
    script_path = os.path.join(WEB_ROOT, 'cumulus-upgrade-ztp.sh')
    key_block = ''
    if pub_key:
        quoted_key = bash_single_quote(pub_key)
        key_block = f'''
mkdir -p /root/.ssh /home/cumulus/.ssh
grep -qxF {quoted_key} /root/.ssh/authorized_keys 2>/dev/null || echo {quoted_key} >> /root/.ssh/authorized_keys
grep -qxF {quoted_key} /home/cumulus/.ssh/authorized_keys 2>/dev/null || echo {quoted_key} >> /home/cumulus/.ssh/authorized_keys
chown -R cumulus:cumulus /home/cumulus/.ssh
chmod 700 /root/.ssh /home/cumulus/.ssh
chmod 600 /root/.ssh/authorized_keys /home/cumulus/.ssh/authorized_keys 2>/dev/null || true
echo "LLDPq SSH key installed"
'''
    content = f'''#!/bin/bash
#
...
#
# This script is generated by LLDPq before each OS upgrade.
# The OS config is preserved by onie-install -t /etc/nvue.d/startup.yaml.

echo "LLDPq upgrade ZTP helper running"
passwd -x 99999 cumulus 2>/dev/null || true
echo {bash_single_quote('cumulus:' + password)} | chpasswd
echo "cumulus ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/10_cumulus
chmod 440 /etc/sudoers.d/10_cumulus 2>/dev/null || true
{key_block}
ztp -d 2>/dev/null || true
echo "LLDPq upgrade ZTP helper complete"
exit 0
'''
    try:
        ...
        subprocess.run(['sudo', '-u', LLDPQ_USER, 'tee', script_path], input=content, capture_output=True, text=True, timeout=10)
    subprocess.run(['sudo', 'chown', f'{LLDPQ_USER}:www-data', script_path], capture_output=True, timeout=5)
    subprocess.run(['sudo', 'chmod', '775', script_path], capture_output=True, timeout=5)
    return bool(pub_key)
...
def action_upgrade_start():
    ...
    key_available = write_upgrade_ztp_script()
    if not key_available:
        error_json('No LLDPq SSH public key found. Generate/import SSH key before upgrade.')
    job = {
        'id': str(uuid.uuid4()),
