#!/usr/bin/env bash
# LLDP Web Trigger / status endpoint
...
# Persist jobs outside the replaceable application tree.  The installer makes
# this setgid directory writable by the LLDPq daemon and fcgiwrap's www-data
# user, so tokens survive application upgrades and daemon restarts.
LLDP_JOB_DIR="${LLDPQ_LLDP_JOB_DIR:-/var/lib/lldpq/lldp-jobs}"
if ! mkdir -p -m 2770 "$LLDP_JOB_DIR" 2>/dev/null; then
    json_error "LLDP job directory is unavailable"
fi
# mkdir honours an existing directory's mode.  Tighten a newly-created or
# older directory when we own it; group membership is the actual access gate.
chmod 2770 "$LLDP_JOB_DIR" 2>/dev/null || true
...
write_initial_status() {
    ...
    temporary=$(mktemp "$LLDP_JOB_DIR/.${token}.status.XXXXXXXX") || return 1
    if ! {
        printf 'token=%s\n' "$token"
        printf 'state=%s\n' "$state"
        printf 'created_at=%s\n' "$created"
       ...
       ! mv -f "$temporary" "$LLDP_JOB_DIR/$token.status"; then
...
if [[ "${REQUEST_METHOD:-GET}" == "GET" ]]; then
    ...
    valid_token "$token" || json_error "Invalid or missing LLDP job token"

    status_file="$LLDP_JOB_DIR/$token.status"
    [[ -f "$status_file" ]] || json_error "LLDP job was not found"

    # Read the inode exactly once.  The daemon publishes with atomic rename;
    # reopening for each field could otherwise mix an old state with fields
    # from the next generation and falsely turn a retry into terminal failure.
    if ! status_snapshot=$(cat -- "$status_file" 2>/dev/null); then
        json_error "LLDP job status is temporarily unavailable"
    fi
    stored_token=$(snapshot_field "$status_snapshot" token)
    state=$(snapshot_field "$status_snapshot" state)
    [[ "$stored_token" == "$token" ]] || json_error "LLDP job status is invalid"
    case "$state" in
        queued|running|success|failure) ;;
        *) json_error "LLDP job status is invalid" ;;
    esac
...
# 128 bits from the kernel RNG: opaque enough to be an unguessable status
# capability, while the strict lowercase-hex format keeps paths inert.
token=""
...
valid_token "$token" || json_error "Could not allocate an LLDP job token"

created_at=$(date +%s)
request_file="$LLDP_JOB_DIR/$token.request"
request_tmp=$(mktemp "$LLDP_JOB_DIR/.${token}.request.XXXXXXXX") || \
    json_error "Could not queue the LLDP job"

# Publish queued status first.  If publishing the request then fails, replace
# the same token's state with a visible terminal failure instead of returning a
# token which can never complete.
if ! write_initial_status "$token" queued "$created_at" "Waiting for the LLDP worker"; then
    rm -f "$request_tmp" 2>/dev/null || true
    json_error "Could not create LLDP job status"
fi
...
if ! {
   ...
   ! mv -f "$request_tmp" "$request_file"; then
    ...
    write_initial_status "$token" failure "$created_at" \
        "Could not publish the LLDP request" 1 || true
    json_error "Could not queue the LLDP job"
fi

printf '{"status":"started","token":"%s","message":"LLDP check queued"}\n' "$token"
