...
def update_notifications(
    ...
    *,
    ...
    try:
        ...
        raise SetupSafetyError("notifications.yaml must be valid UTF-8") from exc
    base_revision = revision_bytes(raw)
    ...
    return atomic_write_text(
        path, rendered, expected_revision=base_revision, managed_roots=managed_roots
    )
...
def _main() -> int:
    ...
    parser.add_argument("command", choices=(
        "save-devices", "save-topology", "save-topology-config", "save-aliases",
    ...
    global_lock_fd = None
    try:
        incoming = _read_stdin_text(MAX_CONFIG_BYTES * 4 if args.request_json else MAX_CONFIG_BYTES)
        content = incoming
        ...
        if args.command == "save-notifications":
            ...
            print(json.dumps({"success": True, "revision": revision}))
            return 0
