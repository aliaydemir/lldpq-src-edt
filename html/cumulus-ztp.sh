...
function main(){
    ...
    if [ -n "$CUMULUS_IMAGE_NAME" ]; then
        if ! [[ "$CUMULUS_IMAGE_NAME" =~ ^[A-Za-z0-9._-]+\.(bin|img|iso)$ ]]; then
            echo "Invalid Cumulus image name" >&2
            return 1
        fi
        IMAGE_SERVER=http://$IMAGE_SERVER_HOSTNAME/$CUMULUS_IMAGE_NAME
    else
        # Empty image name keeps the legacy release-derived stock filename.
